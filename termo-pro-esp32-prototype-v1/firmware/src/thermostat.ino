...
uint8_t receiverMAC[] = { 0xD4, 0x8A, 0xFC, 0xA2, 0xA0, 0xD8 };
...
void change_program(int prog_num) {
    ...
    int cursor = 7;
    lcd.setCursor(0, 0);
    lcd.print("EDITING P["); lcd.print(prog_num); lcd.print("]");
...
void loop() {
    ...
    if(day > 7) { day = 1; set_time(hour, minute, day); }
    ...
    // OO:MM DAY[N] D/N O/F
    lcd.print(timeStr);
    lcd.print(" "); lcd.print("DAY["); lcd.print(day); lcd.print("] ");
    ...
    while (sensor.checkButtonInput() != 0); // wait release
    lcd.clear();
    ...
    // uses an NPN 2N2222 transistor to switch current, and brightness is controled by PWM, D = rez_val / 254 (254 duty cycles in total)
